...
const Chatlist: React.FC<ChatlistProps> = ({ chatUsers }) => {
  ...
  return (
    ...
    {chatUsers.chats.map((chat,index)=>(
        <NavLink key={index} to={`${window.matchMedia('(max-width: 600px)').matches?'/pchat':'/chat'}`}><div className='w-full h-1012  bg-black2 rounded-lg flex justify-between items-center px-3 lg:px-2'>
