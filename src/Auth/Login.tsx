...
console.log(errorMsg)
  return (
    <div  className={`bg-black2 ${fixedBody?'overflow-auto':'overflow-hidden'}   w-full h-full px-4 sm:px-6 md:px-8 lg:px-0 lg:flex justify-center   lg:justify-normal  `}>
      {switchForm ?
        ...
        <div className="lg:px-7 lg:pt-7    overflow-x-hidden   flex flex-col gap-7 sm:gap-10 lg:gap-9 ">
          <div className="flex flex-col  lg:items-center relative    justify-between w-full gap-4 ">
              ...
              <div  className={`w-full text-white  flex sm:text-lg  flex-col justify-around items-center h-103 before:pointer-events-none ${fixedBody?'-top-40 sm:-top-80':' top-16'}  absolute lg:hidden transition-all duration-500   before:h-103 before:w-full before:absolute before:bg-opacity-20 backdrop-blur-2xl`}>
                <NavLink to={'/landingPage/phoneSignin'} className={''}><p className="font-bold text-white z-30 ">Sign In</p></NavLink>
                <NavLink to={'/landingPage/phoneSignup'}><p className="font-bold text-white ">Sign Up</p></NavLink>
          ...
          {middlemanImage&&<motion.div style={{ display: imageLoaded ? "visible" : "invisible" }} className="w-full  rounded-xl h-auto  md:pb-6 md:pr-0 md:pl-6 md:pt-6 mt-3 sm:mt-5 gap-7 sm:gap-10 md:gap-0   md:bg-black  flex flex-col md:flex-row   "  variants={textRevealVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.5 }}>
            <img src={middlemanImage} onLoad={handleImageLoad} className="bg-no-repeat bg-cover bg-center  md:w-105 h-auto   md:rounded-e-none w-full object-cover  "/> 
        ...
        <footer className={` bg-black  text-white px-5 sm:px-14 md:px-10 lg:px-16 mt-3  py-8 sm:py-10  bottom-0 `}>
